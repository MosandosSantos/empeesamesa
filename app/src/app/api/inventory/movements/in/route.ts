...
export async function POST(request: NextRequest) {
  try {
    ...
    if (user.role !== "ADMIN") {
      return NextResponse.json({ error: "Somente admin pode registrar entrada" }, { status: 403 });
    }
    ...
    if (item.archivedAt) {
      return NextResponse.json({ error: "Item arquivado nao recebe entradas" }, { status: 400 });
    }
    ...
    const purchaseValue = qty * unitCost;
    const newQty = currentQty + qty;
    const newAvg = newQty === 0 ? 0 : (currentValue + purchaseValue) / newQty;
    ...
    console.error("POST /api/inventory/movements/in error:", err);
